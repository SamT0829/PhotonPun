...
public class Spawner : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    {
        NetworkHandler.Instance.OnInput(runner, input);
    }
    ...
    public void OnPlayerLeft(NetworkRunner runner, PlayerRef player)
    {
        ...
        Debug.Log($"OnPlayerLeft, Connection token {playerToken}");

        //Check if the token is already recorded by the server. 
        if (mapTokenIDWithNetworkPlayer.TryGetValue(playerToken, out NetworkPlayer networkPlayer))
        {
            Debug.Log($"Found old connection token for token {playerToken}. Remove Player token");

            runner.Despawn(mapTokenIDWithNetworkPlayer[playerToken].Object);
            mapTokenIDWithNetworkPlayer.Remove(playerToken);
        }
        else
        {
            Debug.LogError($"Not Found old connection token for token {playerToken}. Remove Player token");
        }
    }
    ...
    public async void OnHostMigration(NetworkRunner runner, HostMigrationToken hostMigrationToken)
    {
        ...
        FindObjectOfType<NetworkHandler>().StartHostMigration(hostMigrationToken);
